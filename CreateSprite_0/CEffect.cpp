#include "CEffect.h"
#include "CInput.h"
bool    CEffect::Create(W_STR name, T_STR_VECTOR texFileName)
{
    m_csName = name;
...
bool    CEffect::Create(W_STR name,
...
bool CEffect::LoadTexture(T_STR_VECTOR texArray)
{
...
bool    CEffect::LoadTexture(std::wstring texFileName) {
...
bool    CEffect::Frame(float fElapsTime)
{
...
bool    CEffect::Render(ID3D11DeviceContext* pd3dContext) {
    if (m_ptTex == nullptr) return true;
    pd3dContext->PSSetShaderResources(0, 1, &m_ptTex->m_pTextureSRV);
    return true;
};
bool    CEffect::PostRender() { return true; };
bool    CEffect::Release() { return true; };
