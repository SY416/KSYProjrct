...
bool CUIObj::LoadTextureChange(std::wstring texFileName)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = DirectX::CreateShaderResourceView(
                m_pd3dDevice,
                m_texChange->GetImages(),
                m_texChange->GetImageCount(),
                metadata,
                &m_pTextureSRVChange);
            if (SUCCEEDED(hr))
            {
    ...
    if (SUCCEEDED(hr))
    {
        hr = DirectX::LoadFromHDRFile(texFileName.c_str(), &metadata, *m_texChange);
        if (SUCCEEDED(hr))
        {
            hr = DirectX::CreateShaderResourceView(
                m_pd3dDevice,
...
bool CUIObj::CreateVertexBuffer()
{
...
bool CUIObj::CreateVertexShader() {
    ID3DBlob* ppErrorMsg;
...
bool CUINumber::LoadTexture(std::wstring texFileName)
{
    for (int i = 0; i < 10; i++)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                hr = DirectX::CreateShaderResourceView(
                    m_pd3dDevice,
                    m_texArray[i]->GetImages(),
                    m_texArray[i]->GetImageCount(),
                    metadata,
                    &m_pTextureSRVArray[i]);
                if (SUCCEEDED(hr))
                {
                    continue;
                }
