...
Texture* TextureMgr::Load(std::wstring texFileName)
{   
    auto iter = m_list.find(texFileName);
    if (iter != m_list.end())
    {
        return iter->second.get();
    }
    std::shared_ptr<Texture>  pTex = std::make_shared<Texture>();
    if (pTex->Load(m_pd3dDevice, m_pd3dContext, texFileName))
    {
        m_list.insert(std::make_pair(texFileName, pTex));
        return pTex.get();
    }
    return nullptr;
}
