...
struct Tstat {
	...
	int criChance = 100;
	int criDamage = 95;
	friend std::ostream& operator<<(std::ostream& os, const Tstat& t) {
		os << "  HP             " << t.hp << "  MP               " << t.mp
			<< "\n  STR            " << t.strength << "\n  DEX            " << t.dexterity << "\n  INT            " << t.intelligence << "\n  LUK            " << t.luck
			<< "\n  공격력         " << t.attack << "    방어력           " << t.defence
			<< "\n  크리티컬 확률  " << t.criChance << " %" << "   크리티컬 데미지  " << t.criDamage << " % " << std::endl;
		return os;
	}
};
...
public:
	...
	friend std::ostream& operator<<(std::ostream& os, const TStatus& t);
};
